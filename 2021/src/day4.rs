...
fn mark_number(card: &[Vec<BingoNumber>], n: u32) -> Vec<Vec<BingoNumber>> {
    card.iter()
        .map(|l| {
            l.iter()
                .map(|i| {
...
fn is_winner(card: &[Vec<BingoNumber>]) -> bool {
    card.iter().any(|l| l.iter().all(|n| n.marked))
        || (0..5).any(|i| card.iter().all(|l| l[i].marked))
}

fn calculate_score(card: &[Vec<BingoNumber>], win_number: u32) -> u32 {
    card.iter().fold(0, |acc, l| {
        acc + l
            .iter()
            .fold(0, |l_acc, n| if !n.marked { l_acc + n.n } else { l_acc })
    }) * win_number
}
...
pub fn part1() {
    ...
    for n in numbers {
        bingo_cards = bingo_cards
            .iter()
            .map(|card| mark_number(card, n))
            .collect();
        let winner = bingo_cards.iter().find(|card| is_winner(card));
...
pub fn part2() {
    ...
    for n in numbers {
        ...
        bingo_cards.retain(|card| !is_winner(card));
    }
