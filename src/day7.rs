...
pub fn part1() {
    ...
    let gas_count = coords.into_iter().fold(0, |acc, e| {
        acc + (e - best_y as i32).abs()
    });
    println!("min gas : {}", gas_count);
}

fn calculate_fuel_cost(from: i32, to: i32) -> i32 {
    (from..=to).enumerate().fold(0, |acc, (i, _)| acc + i as i32)
}

fn mean(numbers: &Vec<i32>) -> f32 {
    numbers.into_iter().sum::<i32>() as f32 / numbers.len() as f32
}
...
pub fn part2() {
    ...
    let gas_count = (min_coord..=max_coord).map(|y| {
        (&coords).into_iter().fold(0, |acc, e| {
            let min = min(y, *e);
            let max = max(y, *e);
